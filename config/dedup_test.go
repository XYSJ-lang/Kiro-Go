...
func TestAddAccountDeduplicatesByUserId(t *testing.T) {
	...
	saved, err := AddAccount(first)
	if err != nil {
		t.Fatalf("add first: %v", err)
	}
	if saved.ID != "id-1" {
		t.Fatalf("expected returned ID id-1, got %q", saved.ID)
	}
	...
	if saved.ID != "id-1" {
		t.Fatalf("dedup should preserve existing ID id-1, got %q", saved.ID)
	}
...
func TestAddAccountKeepsDifferentUserIdsSeparate(t *testing.T) {
	...
	if _, err := AddAccount(Account{ID: "a", Email: "user@example.com", UserId: "google-uid", AuthMethod: "social"}); err != nil {
		t.Fatalf("add google: %v", err)
	}
	if _, err := AddAccount(Account{ID: "b", Email: "user@example.com", UserId: "github-uid", AuthMethod: "social"}); err != nil {
...
func TestAddAccountAlwaysAppendsEmptyUserId(t *testing.T) {
	...
	if _, err := AddAccount(Account{ID: "x", Email: "a@example.com", UserId: "", AuthMethod: "idc"}); err != nil {
		t.Fatalf("add first empty: %v", err)
	}
	if _, err := AddAccount(Account{ID: "y", Email: "b@example.com", UserId: "", AuthMethod: "idc"}); err != nil {
